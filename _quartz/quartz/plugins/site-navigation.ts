...
export function folderTitle(slug: string): string {
  ...
  const folder =
    segments.at(-1) === "index" ? segments.at(-2) : segments.at(-1);
...
export function assertRequiredPageTypes(config: QuartzConfig): void {
  const names = new Set(
    config.plugins.pageTypes?.map((pageType) => pageType.name),
  );
  for (const name of ["ContentPage", "FolderPage", "TagPage"]) {
    if (!names.has(name))
      throw new Error(`Required Quartz page type did not load: ${name}`);
  }
}
...
function readableHeading(
  heading: MarkdownRoot["children"][number],
): string | undefined {
  return (
    toString(heading, { includeHtml: false }).replace(/\s+/g, " ").trim() ||
    undefined
  );
}
...
export async function configureSiteNavigation(config: QuartzConfig): Promise<{
  ...
  const [{ componentRegistry }, { PageTypeDispatcher }, { loadQuartzLayout }] =
    await Promise.all([
  ...
  const folderPage = config.plugins.pageTypes!.find(
    (pageType) => pageType.name === "FolderPage",
  )!;
  const generateFolders = folderPage.generate;
  if (!generateFolders)
    throw new Error("FolderPage cannot generate folder pages");
  ...
  if (frontmatterIndex < 0)
    throw new Error("Required NoteProperties transformer did not load");
  ...
  const { default: explorerControls } =
    await import("./site-explorer.inline.ts");
  ...
  if (graph) {
    if (typeof graph.component !== "function")
      throw new Error("Quartz Graph is not a component");
    const originalGraph = graph.component as QuartzComponentConstructor<
      Record<string, unknown>
    >;
    // @ts-expect-error Quartz's inline-script loader imports this browser module as text.
    const { default: graphControls } = await import("./site-graph.inline.ts");
    const wrappedGraph: QuartzComponentConstructor<Record<string, unknown>> = (
      options,
    ) => {
  ...
  for (const [registryName, displayName] of Object.entries(railTags)) {
    ...
    const original = entry.component as QuartzComponentConstructor<
      Record<string, unknown>
    >;
    const tagged: QuartzComponentConstructor<Record<string, unknown>> = (
      options,
    ) => {
  ...
  const originalArticleTitle =
    articleTitle.component as QuartzComponentConstructor;
  componentRegistry.register(
    "article-title",
    (options: undefined) => {
      const source = originalArticleTitle(options);
      const component: QuartzComponent = (props) =>
        props.fileData.sourceHeadingTitle && !props.fileData.hoistedHeading
          ? null
          : source(props);
  ...
  if (dispatcherIndex < 0)
    throw new Error("Required Quartz page dispatcher did not load");
