...
function resolveConfigPath(): string {
  // QUARTZ_CONFIG points a build at an alternative configuration file, used
  // for design-variant preview builds. Production leaves it unset.
  const override = process.env.QUARTZ_CONFIG?.trim();
  if (override) {
    const resolved = path.resolve(process.cwd(), override);
    if (!fs.existsSync(resolved))
      throw new Error(`QUARTZ_CONFIG not found: ${resolved}`);
    return resolved;
  }
  ...
  const instantiate = async (
    ...
    for (const { entry, manifest } of items) {
      try {
        ...
        instances.push(
          factory(Object.keys(options).length > 0 ? options : undefined),
        );
      } catch (err) {
        console.error(
          styleText("red", `✗`) +
...
function matchesCategory(
  ...
  try {
    ...
    switch (expected) {
      case "pageType":
        return (
          "match" in instance && "body" in instance && "layout" in instance
        );
      case "emitter":
...
function findFactory(
  ...
  for (const [, value] of Object.entries(module)) {
    if (
      ...
      matchesCategory(value as Function, expectedCategory)
    ) {
...
function buildLayoutForEntries(
  ...
  for (const entry of entries) {
    ...
    const registered =
      ...
      componentRegistry.get(`${entry.source}/${name}`);
    if (!registered) {
...
function resolveGroups(
  ...
  for (const item of items) {
    if (item.group) {
      ...
      entries.push({
        priority: groupPriority.get(item.group)!,
