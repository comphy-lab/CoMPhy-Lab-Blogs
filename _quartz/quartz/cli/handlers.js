...
`,
  ...
  if (!configExists()) {
    if (template && template !== "default") {
      ...
      console.log(
        styleText("green", "Created quartz.config.yaml from defaults"),
      );
    }
  }
...
export async function handleBuild(argv) {
  ...
  const ctx = await esbuild.context({
    ...
    plugins: [
      {
        // QUARTZ_THEME swaps the site stylesheet for quartz/styles/themes/<name>.scss.
        // Used by design-variant preview builds; production leaves it unset.
        name: "theme-stylesheet",
        setup(build) {
          const theme = process.env.QUARTZ_THEME?.trim();
          if (!theme) return;
          const themePath = path.resolve(
            "quartz/styles/themes",
            `${theme}.scss`,
          );
          if (!fs.existsSync(themePath)) {
            throw new Error(`QUARTZ_THEME stylesheet not found: ${themePath}`);
          }
          build.onResolve({ filter: /styles\/custom\.scss$/ }, () => ({
            path: themePath,
          }));
        },
      },
